...
PLUGIN_SLUG = "shinonomeiro-headless-bridge"
PLUGIN_FILE = ROOT_DIR / "plugins" / PLUGIN_SLUG / f"{PLUGIN_SLUG}.php"
...
PLUGIN_EXCLUDE_DIRS = {".git", "node_modules"}
...
def main() -> int:
    parser = argparse.ArgumentParser(description="构建主题与插件的 Release 资产。")
    ...
    plugin_dir = ROOT_DIR / "plugins" / PLUGIN_SLUG

    theme_version = read_version(STYLE_FILE, r"^Version:\s*(.+)$", "未能从 style.css 解析主题版本")
    plugin_version = read_version(PLUGIN_FILE, r"^ \* Version:\s*(.+)$", "未能从插件主文件解析插件版本")
    ...
    print("构建插件发布目录...")
    copy_tree(plugin_dir, plugin_stage, PLUGIN_EXCLUDE_DIRS, ())

    theme_zip = output_dir / f"{THEME_SLUG}-v{theme_version}.zip"
    plugin_zip = output_dir / f"{PLUGIN_SLUG}-v{plugin_version}.zip"

    print("打包主题 ZIP...")
    zip_dir(stage_dir, theme_stage, theme_zip)
    print("打包插件 ZIP...")
    zip_dir(stage_dir, plugin_stage, plugin_zip)
